...
3. **Secrets 設定（本番）**
   ...
   `.dev.vars` の中身を Cloudflare 側へ一括投入できる（非シークレットの `OAUTH_REDIRECT_URL` は除外される）:
   ```bash
   pnpm secrets:push          # 既定環境へ
   pnpm secrets:push:prod     # production 環境へ（--env production）
   ```
   個別に入れる場合:
   ```bash
   ...
   ```
   > `.dev.vars` はローカル `wrangler dev` 専用。デプロイ後の Worker はこのファイルを読まないため、シークレットは必ず Cloudflare 側へアップロードする。

4. **デプロイ**
   ```bash
   pnpm deploy        # 既定環境（wrangler deploy）
   pnpm deploy:prod   # production 環境（wrangler deploy --env production）
   ```
   `production` 環境は `wrangler.jsonc` の `env.production` に定義済み。Worker 名は `live-reserve-production` になるため、`OAUTH_REDIRECT_URL` を本番URL（`https://live-reserve-production.<subdomain>.workers.dev/api/auth/callback`）に書き換え、Google Cloud の承認済みリダイレクトURIにも追加すること。
   > 名前付き環境では `vars` / `d1_databases` / `r2_buckets` は継承されないため、`env.production` 側にも同じ内容を記載してある（`database_id` は本番用に差し替える）。本番DBには `wrangler d1 migrations apply live-reserve --remote --env production` を適用。
