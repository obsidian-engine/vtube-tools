...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ツールカードのホバー効果はCSS側で制御（MD3準拠のelevation変化）
  ...
  const animationTargets = document.querySelectorAll(
    ".tool-card, .feature-item, .stats-item",
  );
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (copyCommentCSSButton) {
      copyCommentCSSButton.addEventListener("click", async function (e) {
        ...
        this.disabled = true;
        this.innerHTML =
          '<span class="material-symbols-outlined btn-icon" aria-hidden="true">hourglass_empty</span> コピー中...';
        ...
        if (result.success) {
          // 成功フィードバック
          this.innerHTML =
            '<span class="material-symbols-outlined btn-icon" aria-hidden="true">check_circle</span> コピー完了！';
          ...
          // エラーフィードバック
          this.innerHTML =
            '<span class="material-symbols-outlined btn-icon" aria-hidden="true">error</span> コピー失敗';
